...
**Marginalia (浮签)** 是一款基于 Obsidian 的沉浸式状态栏插件。它利用屏幕底部的状态栏空间，通过「一言」(Hitokoto) / Quotable（失败时回退到 ZenQuotes）或本地自定义语录库，展示格言、名言或灵感片段。
...
* 📝  **多种数据源** ：支持「[一言](https://hitokoto.cn/)」(Hitokoto)、[Quotable](https://quotable.io/)（含 [ZenQuotes](https://zenquotes.io/) 回退）和本地自定义列表。
...
在官方商店审核通过前，推荐使用 [BRAT](https://github.com/TfTHacker/obsidian42-brat "null") 插件安装：
...
* 当使用 **[一言](https://hitokoto.cn/) API** 作为数据源时，插件会定期访问 `https://v1.hitokoto.cn/` 获取随机格言。
* 当使用 **[Quotable](https://quotable.io/) API** 作为数据源时，插件会访问 `https://api.quotable.io/random`（必要时回退到来自 [ZenQuotes](https://zenquotes.io/) 的 `https://zenquotes.io/api/random`）。
...
* **[「一言」](https://hitokoto.cn/) API** ：联网获取随机格言。
* **[Quotable](https://quotable.io/) API** ：联网获取随机名言（含 [ZenQuotes](https://zenquotes.io/) 回退）。
...
1. **配置自动部署环境 (可选)：** 本项目支持在开发时自动将构建好的插件同步到你的 Obsidian 测试库。
...
Marginalia/
├── src/
│   ├── services/           # 在线语录服务 + 本地语录管理
│   │   ├── hitokoto.ts     # 一言 API（含分类参数）
│   │   ├── quotable.ts     # Quotable API（含 ZenQuotes 回退）
│   │   └── quoteManager.ts # 本地语录解析与随机抽取
│   ├── ui/
...
* **作者：**Open Open
* **GitHub：** [OpeNopEn2007](https://github.com/OpeNopEn2007 "null")

## 📅 更新日志

**v1.1.0**

* ✨ **新特性：** 新增 Quotable API 数据源（含 ZenQuotes 回退）。
* ⚙️ **设置：** 新增数据源选择（「一言」/ Quotable / 本地自定义列表）。
* 🌐 **语言：** 根据界面语言优化默认数据源（中文 → 一言，其他 → Quotable）。
* 📝 **文档：** 同步更新网络访问说明与项目结构。
